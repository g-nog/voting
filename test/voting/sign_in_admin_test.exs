defmodule Voting.SignInAdminTest do
  ...
  describe "run/2" do
    ...
    test "returns error when there is no admin with this email" do
      assert {:error, :email_or_password_invalid} = SignInAdmin.run("zz@example.com", "123456")
    end

    test "returns error when the password is invalid" do
      assert {:error, :email_or_password_invalid} = SignInAdmin.run("email@example.com", "wrong")
    end
  end
end
